...
def main():
    ...
    agent1 = LenientDQNAgent(env.envs[0], [256, 256], 'LenientAgent1',
                             learning_rate=1e-4,
                             ...
                             ts_greedy_coeff=0.25)

    agent2 = LenientDQNAgent(env.envs[0], [256, 256], 'LenientAgent2',
                             learning_rate=1e-4,
                             use_tau=True, tau=1e-3,
        ...
        while len(ep_len_log) < TRAIN_EPISODES:
            ...
            for j in range(ENV_NUM):
                for k in range(STEP_N):
                    action_1 = agent1.choose_action(states_1[j])
                    action_2 = agent2.choose_action(states_2[j])
                    ...
                    ln_1[j].append(
                        agent1.leniency_calculator.calc_leniency(agent1.temp_recorder.get_state_temp(states_1[j])))
                    ...
                    ln_2[j].append(
                        agent2.leniency_calculator.calc_leniency(agent2.temp_recorder.get_state_temp(states_2[j])))
                    ...
                    if done_1:
                        ...
                        agent1.temp_recorder.decay_temp(episodes_1[j])
                        agent2.temp_recorder.decay_temp(episodes_2[j])

                        ep_cnt += 1

                        this_train_log = (train_num, ep_cnt, j,
                                          agent1.temp_recorder.get_ave_temp(),
                                          agent1.temp_recorder.get_temp_len(),
                                          len(episodes_1[j]))
                        train_log.append(this_train_log)

                        print('train_num: {}, episode_cnt: {}, env: {} , mean_temp: {}, temp_len: {}, len: {} '.format(
                            *this_train_log))
                        agent1.temp_recorder.show_temp(big=False)
